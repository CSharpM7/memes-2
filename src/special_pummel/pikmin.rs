...
pub unsafe extern "C" fn catch_attack_exec(fighter: &mut L2CFighterCommon) -> L2CValue {
    if WorkModule::count_down_int(fighter.module_accessor, THROW_TIMER, 0) {
        let olima = fighter.global_table[MODULE_ACCESSOR].get_ptr() as *mut FighterModuleAccessor;
        ...
        let p = WorkModule::get_int(fighter.module_accessor, THROW_CYCLE);
        let could_throw = throw_pikmin(fighter,p);
        if could_throw {
            WorkModule::set_int(fighter.module_accessor, 5, THROW_TIMER);
        }
        WorkModule::inc_int(fighter.module_accessor, THROW_CYCLE);
    }
    return 0.into();
}
